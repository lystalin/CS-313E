...
def groupSum(start, nums, target):
    # checks if there are nums still in range to check
    if start >= len(nums):
        # if some numbers add to target
        return (target == 0)
    else:
    # if next numbers either can be in the target or not
...
def groupSum5(start, nums, target):
    ...
    if start >= len(nums):
        # if some numbers add to target
        return (target == 0)
    # if index value of nums is multiple of 5
    elif nums[start] % 5 == 0:
...
def groupSumClump(start, nums, target):
    ...
    if start >= len(nums):
        # if some numbers add to target
        return (target == 0)
    elif start+1 < len(nums) and nums[start] == nums[start+1]:
...
# helper funtions that takes in a start index, a placeholder x group, a placeholder y group, and nums array
# returns a boolean
def splitArrayHelper(start, x, y, nums):
    # checks if there are nums still in range to check
    if start >= len(nums):
        #if sum of group x is equivalent to sum of group y
        return (x == y)
    else:
        return splitArrayHelper(start + 1, x + nums[start], y, nums) or splitArrayHelper(start + 1, x, y + nums[start], nums)
...
# helper funtions that takes in a start index, a placeholder x group, a placeholder y group, and nums array
# returns a boolean 
def splitOdd10Helper(start, x, y, nums):
    # checks if there are nums still in range to check
    if start >= len(nums):
        # if x group is divisible by ten and y group is prime
        return (x % 10 == 0 and y % 2 != 0)
    else:
        return splitOdd10Helper(start + 1, x + nums[start], y, nums) or splitOdd10Helper(start + 1, x, y + nums[start], nums)
...
# helper funtions that takes in a start index, a placeholder x group, a placeholder y group, and nums array
# returns a boolean 
def split53Helper(start, x, y, nums):
    # checks if there are nums still in range to check
    if start >= len(nums):
        # if sum of x and sum of y are equivalent
        return (x == y)
    # if index value of nums is divisible by 5
    elif nums[start] % 5 == 0:
        return split53Helper(start+1, x + nums[start], y, nums)
    # if index value of nums is divisble by 3
    elif nums[start] % 3 == 0:
        return split53Helper(start+1, x, y + nums[start], nums)
    else:
        return split53Helper(start+1, x, y + nums[start], nums) or split53Helper(start+1, x + nums[start], y, nums)
